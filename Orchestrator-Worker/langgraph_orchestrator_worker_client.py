...
async def main():
	async with streamable_http_client("http://localhost:5000/mcp") as (read, write, stdio):
		async with ClientSession(read, write) as session:
			...
			graph.add_node("tavily", tavily_agent)
			graph.add_node("llm_knowledge_base", knowledge_base_agent)
			...
			graph.add_conditional_edges("router", router, [AgentType.TAVILY.value, AgentType.KNOWLEDGE_BASE.value])
			graph.add_edge("tavily", "result")
			graph.add_edge("llm_knowledge_base", "result")
